...
const FeedScreen = ({ navigation, route }) => {
    ...
    const renderScene = ({ route }) => {
        if (data[route.key]) {
            return <FeedView status={status} navigation={navigation} children={children} data={data[route.key]} onRefresh={onRefresh} refreshing={refreshing[route.key]} feed_type={route.key} />
        }
        else {
            return <PostLoader />
        }
